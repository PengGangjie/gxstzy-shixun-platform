...
| 应用名 | `gxstzy-shixun-platform`（显示名；原曾叫 `gxstzy-shixun-v2`。已删废应用同名旧实例 / `p57b49…`） |
| 类型 | Traditional Web |
| App ID | `bbmxu4hpufzkorzhilvl9`（以控制台为准；换名不换 ID） |
...
## 四点五、GitHub 可见性（2026-08-20）

| 项 | 值 |
|----|-----|
| 仓库 | https://github.com/PengGangjie/gxstzy-shixun-platform |
| 可见性 | **PRIVATE**（暂停公开） |
| 影响 | Space 现网可继续跑；**重部署**需临时改回 Public，或教员私有仓通道 |
| RBAC 设计 | `docs/RBAC落地设计_20260820.md` |
...
| 仓库 | https://github.com/PengGangjie/gxstzy-shixun-platform（已 PRIVATE） |
| 分支 | `main` |
| 端口 | `8000` |
| 首次部署 | 2026-08-19 |
| 免费托管至 | 2027-08-19 |
...
| 已修复 | Logto `invalid_client` | 曾改用重建应用（显示名一度为 v2，后改回 `gxstzy-shixun-platform`）/ App ID `bbmxu4hpufzkorzhilvl9`；废应用已删 |
